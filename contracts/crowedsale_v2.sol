...
contract BCT_ICO is ReentrancyGuard, Context, Ownable {
  ...
  mapping(address => uint256) public referrersFee;
  ...
  function _buyTokens(address beneficiary, address ref) internal {
    ...
    if (ref != address(0)) {
        uint256 refTokens = _getTokenAmountRef(tokens, ref);
  ...
  function tranferFrozInvest(address whom, uint256 amount, address ref) public onlyOwner returns (bool) {
	token.transfer(whom, amount);
    if (ref != address(0)) {
        uint256 refTokens = _getTokenAmountRef(amount, ref);
        token.transfer(ref, refTokens);
        referrers[ref] = referrers[ref].add(refTokens);
    }
	return true;
  ...
  function _getTokenAmountRef(uint256 weiAmount, address ref) internal view returns (uint256){
	if (referrersFee[ref] > 0 )
		return weiAmount.mul(referrersFee[ref]).div(100);
    return weiAmount.mul(refPrecent).div(100);
  }
  ...
  function checkReferrerFee(address addr) public view returns (uint256) {
    return referrersFee[addr];
  }

  function setReferrerFee(address account, uint256 camount) public onlyOwner {
	referrersFee[account] = camount;
  }
